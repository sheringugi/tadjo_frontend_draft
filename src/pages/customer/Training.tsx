      ...
      <div className="container mx-auto">
        ...
        <section className="grid md:grid-cols-2 min-h-[480px] border border-border overflow-hidden">
          ...
          <div className="p-8 md:p-16 flex flex-col justify-center bg-background">
            <p className="text-xs tracking-wide-luxury uppercase text-muted-foreground mb-4">{t('training.heroSub')}</p>
            <h1 className="text-5xl md:text-6xl font-display font-normal text-foreground mb-6 leading-tight">
              {t('training.heroHeading')}
            </h1>
            <p className="text-muted-foreground mb-8 max-w-md leading-relaxed">
              {t('training.heroDescription')}
            </p>
            <div className="text-2xl font-display font-normal mb-8">
              CHF 350 <span className="text-sm font-normal text-muted-foreground ml-2">· {t('training.priceSuffix')}</span>
            </div>
        ...
        <section className="py-24 border-x border-border">
          <p className="text-xs tracking-wide-luxury uppercase text-muted-foreground mb-12 text-center">{t('training.howItWorksHeading')}</p>
          <div className="grid md:grid-cols-3">
            {[
              { num: "01", title: t('training.step1Title'), desc: t('training.step1Desc') },
              { num: "02", title: t('training.step2Title'), desc: t('training.step2Desc') },
              { num: "03", title: t('training.step3Title'), desc: t('training.step3Desc') },
            ].map((step, i) => (
        ...
        <section className="py-24 border border-border">
          <p className="text-xs tracking-wide-luxury uppercase text-muted-foreground mb-12 text-center">{t('training.issuesHeading')}</p>
        ...
        <section className="py-24 border-x border-b border-border mb-24">
          <p className="text-xs tracking-wide-luxury uppercase text-muted-foreground mb-12 text-center">{t('training.includedHeading')}</p>
          <div className="grid lg:grid-cols-5 gap-12 px-8 md:px-16">
            <div className="lg:col-span-2 space-y-6">
              ...
              <p className="text-muted-foreground leading-relaxed">
                {t('training.includedDescription')}
              </p>
              <div className="pt-6 border-t border-border flex items-start gap-4 text-xs text-muted-foreground leading-relaxed italic">
                ...
                <span>{t('training.videoNote')}</span>
              </div>
            ...
            <div className="lg:col-span-3">
              <ul className="divide-y divide-border">
                {[
                  { icon: Calendar, name: t('training.itemAssessmentTitle'), meta: t('training.itemAssessmentMeta'), desc: t('training.itemAssessmentDesc') },
                  { icon: Video, name: t('training.itemSessionsTitle'), meta: t('training.itemSessionsMeta'), desc: t('training.itemSessionsDesc') },
                  { icon: CheckCircle2, name: t('training.itemReviewTitle'), meta: t('training.itemReviewMeta'), desc: t('training.itemReviewDesc') },
                  { icon: Mail, name: t('training.itemSupportTitle'), meta: t('training.itemSupportMeta'), desc: t('training.itemSupportDesc') },
                ].map((item) => (
          ...
          >
            ...
            >
              ...
              {!submitted ? (
                <>
                  <div className="mb-10">
                    <h2 className="text-3xl font-display font-normal mb-2 text-foreground"><Trans i18nKey="training.modalHeading" components={{ em: <em /> }} /></h2>
                    <p className="text-sm text-muted-foreground italic font-light">{t('training.modalSub')}</p>
                  </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    >
                      ...
                      {submitting ? t('common.sending', { defaultValue: 'Sending...' }) : t('training.formButton')}
                    </Button>
                  </form>
                </>
              ) : (
                <div className="text-center py-8">
                  ...
                  <h3 className="text-2xl font-display mb-4">{t('training.successHeading')}</h3>
                  <p className="text-muted-foreground font-light italic leading-relaxed">
                    <Trans i18nKey="training.successDescription" components={{ br: <br /> }} />
                  </p>
